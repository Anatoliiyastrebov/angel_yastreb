...
3. Консультант смотрит PDF/изображения в **админке** на странице заявки (`/admin/submissions/<id>`): файл отдаётся через `GET .../attachments/view` (прокси на сервере, `no-store`), без открытой подписанной ссылки Supabase.  
